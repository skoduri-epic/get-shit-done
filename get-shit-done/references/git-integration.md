...
3. **Independent commits:** Each sub-repo receives its own atomic commit via `gsd-tools.js commit-to-subrepo`. File paths are made relative to the sub-repo root before staging.
...
```bash
node ~/.claude/get-shit-done/bin/gsd-tools.js commit-to-subrepo "feat(02-01): add user API" \
